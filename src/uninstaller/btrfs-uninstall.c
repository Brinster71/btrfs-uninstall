 ...
 * Simple wrapper to execute the uninstall-winbtrfs.ps1 script
 */
...
#define SCRIPT_NAME L"uninstall-winbtrfs.ps1"
...
int wmain(int argc, WCHAR* argv[]) {
    ...
    if (GetModuleFileNameW(NULL, exePath, MAX_PATH) == 0) {
        ShowError(L"Failed to get executable path.");
        return 1;
    }
    ...
    _snwprintf(scriptPath, MAX_PATH, L"%s\\%s", exePath, SCRIPT_NAME);
    
    // Check if the script exists
    if (GetFileAttributesW(scriptPath) == INVALID_FILE_ATTRIBUTES) {
        WCHAR errorMsg[512];
        _snwprintf(errorMsg, 512, 
                   L"Uninstall script not found:\n%s\n\n"
                   L"Please ensure uninstall-winbtrfs.ps1 is in the same directory as this executable.",
                   scriptPath);
        ShowError(errorMsg);
        return 1;
    }
